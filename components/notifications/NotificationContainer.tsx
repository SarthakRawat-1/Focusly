...
export const NotificationContainer = ({ userId }: Props) => {
  ...
  const {
    ...
    // Force fresh data on mount to avoid cache issues
    staleTime: 0,
  });
            ...
            <div className="flex flex-col gap-6">
              ...
              >
                <div className="flex flex-col gap-3 h-full">
                  {userNotifications?.map((notify, index) => {
                  ...
                  {/* Fallback if no notifications render */}
                  {userNotifications?.length > 0 && 
                   userNotifications.every(notify => !notify || typeof notify !== 'object' || !notify.id) && (
